...
if __name__ == '__main__':
    ...
    item_file = pd.read_csv(train_item_file, sep='\t')
    item = item_file["item_id:token"]

    unique_item_set = set(item)
    train_item = sorted(list(unique_item_set)) 


    train_item_list = []
    for item in train_item:

        item_index = id_mapping['item2id'][str(item)] - 1
