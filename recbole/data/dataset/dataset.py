...
class Dataset(torch.utils.data.Dataset):
    ...
    def __init__(self, config):
        ...
        if config["expand_final"]:
            self.expand_final = config['expand_final']
            self.item2code , self.code2item = self._load_item2tokens()
            self.token2code, self.code2token = self.load_code()

    def load_code(self):

        token2code = self.item2code

        mapped_feat = np.zeros((self.item_num, self.n_digit), dtype=np.int32)

        code2internal = {} 

        for i, external_item_id in enumerate(self.field2id_token['item_id']):
            if external_item_id == '[PAD]':
                continue
            code = token2code.get(external_item_id)
            mapped_feat[i] = code
            code2internal[tuple(code)] = i  

        return mapped_feat,code2internal
    ...
    def _load_item2tokens(self):
        ...
        if self.dataset_name == "instrument":
            sem_ids_path = ""
