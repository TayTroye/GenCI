...
class ProductVectorQuantizer(nn.Module):
    ...
    def __init__(self, codebook_num, codebook_size, codebook_dim, dist ='cos', tau=0.2, vq_ema=False,dnn_hidden_units=[128],net_dropout=0.1,batch_norm=True):
        super().__init__()
        ...
        # self.dnn = MLP_Block(input_dim=self.codebook_dim * self.codebook_num,
        #                                  output_dim= self.codebook_dim * self.codebook_num, # output hidden layer
    ...
    def forward(self, x):
        ...
        x_chunk = torch.chunk(x, self.codebook_num, dim=-1) #分为codebook_num块
        for idx, quantizer in enumerate(self.vq_layers):
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sk_epsilon > 0 and self.training:
            ...
            indices = torch.argmax(Q, dim=-1)  #argmax
        ...
        # x_q = x + (x_q - x).detach() #在单阶段不使用detach()
