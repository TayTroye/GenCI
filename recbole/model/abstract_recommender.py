...
class ContextRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        self.total_feature_num = (
            len(self.token_field_names)
            + len(self.token_seq_field_names)
            + len(self.float_field_names)
            + len(self.float_seq_field_names)
        )


    def embed_seq(self,item_seq):
        B, L = item_seq.shape

     

        if 'item_id' in self.token_field_names:
            field_idx = self.token_field_names.index('item_id')
            offset = self.token_field_offsets[field_idx]

            item_id_tensor = item_seq + offset
            item_id_tensor = item_id_tensor.view(B,L)


            item_id_embedding = self.token_embedding_table.embedding(item_id_tensor)  


        return item_id_embedding
    ...
    def embed_input_fields_ui(self, interaction):
        ...
        user_feature_names = {'user_id'}
        item_feature_names = {'item_id'}
        item_seq_feature_names = {'categories'}

        token_field_order = ['user_id', 'item_id']

        user_indices_in_order = []
        item_indices_in_order = []
        for i, field_name in enumerate(token_field_order):
            if field_name in user_feature_names:
                user_indices_in_order.append(i)
            elif field_name in item_feature_names:
                item_indices_in_order.append(i)


        ordered_token_tensors = [interaction[name].unsqueeze(1) for name in token_field_order]

        all_tokens_tensor = torch.cat(ordered_token_tensors, dim=1)
        

        all_token_embeddings = self.embed_token_fields(all_tokens_tensor)

        user_token_embedding = all_token_embeddings[:, user_indices_in_order, :]
        item_token_embedding = all_token_embeddings[:, item_indices_in_order, :]
        

        
        user_embedding = user_token_embedding 


        item_token_seq_embedding = None
        item_token_seq_fields = [
            interaction[name] for name in item_seq_feature_names if name in self.token_seq_field_names
        ]
        if item_token_seq_fields:
            item_token_seq_embedding = self.embed_token_seq_fields(item_token_seq_fields)

        if item_token_seq_embedding is not None:
            item_embedding = torch.cat([item_token_embedding, item_token_seq_embedding], dim=1)
        else:
            item_embedding = item_token_embedding

        sparse_embedding = {'user': user_embedding, 'item': item_embedding}

        return sparse_embedding, dense_embedding


    def embed_input_fields_ui_ml(self, interaction):
        """Embed the whole feature columns.
        #针对ml的
        Args:
            interaction (Interaction): The input data collection.
        ...
        user_feature_names = {'user_id', 'age', 'gender', 'occupation'}
        item_feature_names = {'item_id', 'release_year'}
        item_seq_feature_names = {'genre'} 

        token_field_order = ['user_id', 'item_id', 'age', 'gender', 'occupation', 'release_year']
    ...
    def embed_input_fields(self, interaction):
        ...
        Returns:
            ...
            torch.FloatTensor: The embedding tensor of float sequence columns.
        """
        # print("self.float_filed" , self.float_field_names)
        # print("self.token_filed_filed" , self.token_field_names)
        float_fields = []
        for field_name in self.float_field_names:
            if len(interaction[field_name].shape) == 3:
                float_fields.append(interaction[field_name])
